...
namespace PSD.PSDGamepkg.Mint
{
    ...
    public class Mint : PSD.Base.Flow.MintBase
	{
        ...
        public override string ToString()
        {
            ...
            foreach (var pair in dict)
            {
                ...
                {
                    var result = ((System.Collections.IEnumerable)value).Cast<object>().ToList();
    ...
    public class CardOutOfPile : Mint
    {
        ...
        public CardOutOfPile(char pile, int count) : base()
        {
    ...
    public class CardOutOfDise : Mint
    {
        ...
        public override MintType MintType { get { return MintType.UI_ONLY; }}
        public CardOutOfDise(char pile, int count) : base()
        {
            Set("pile", pile.ToString()).Set("count", count);
        }
        public override string ToMessage()
    ...
    public class Stargazer : Mint
    {
        ...
        // case 1: standard watch
        public static Stargazer NewWatch(ushort operater,
        ...
        public static Stargazer Parse(string message)
        {
            ...
            if (show == 0)
            {
                ...
                ushort[] cards = Util.TakeRange(g2fu, 5, g2fu.Length)
                    .Select(p => ushort.Parse(p)).ToArray();
                return Stargazer.NewWatch(op, views, cardType, cards);
            }
            else if (show == 1)
            {
        ...
        public override void Handle(XI xi)
        {
            ...
            if (show == "watch")
            {
                ...
                if (op != 0)
                    xi.WI.Send("E0FU,4," + cardType + "," + string.Join(",", cards), 0, op);
                xi.WI.Send("E0FU,0," + cardType + "," + string.Join(",", cards),
                    invs.Except(new ushort[] { op }).ToArray());
                xi.WI.Send("E0FU,1," + cardType + "," + cards.Count, xi.ExceptStaff(invs));
                xi.WI.Live("E0FU,1," + cardType + "," + cards.Count);
            }
            else if (show == "show")
            {
                ...
                char cardType = GetString("cardType")[0];
                List<ushort> cards = GetUshortArray("cards");
                xi.WI.BCast("E0FU,2," + owner + "," + cardType + "," + string.Join(",", cards));
            }
            else if (show == "close")
                xi.WI.BCast("E0FU,3");
            // else if (show == "take")
            ...
            //     char cardType = GetString("cardType")[0];
            //     List<ushort> cards = GetUshortArray("cards");
            //     ushort[] invs = (views ?? xi.Board.Garden.Keys).ToArray();
    ...
    // someone take away cards from stargaze table
    public class Starshards : Mint
    {
        public override string Head { get { return "G2QU"; } }
        public override MintType MintType { get { return MintType.UI_ONLY; }}
        public Starshards(IEnumerable<ushort> views, char cardType, ushort card) : base()
        {
            if (views != null)
                Set("views", views.ToList());
            Set("cardType", cardType.ToString()).Set("cards", new ushort[] { card });
        }
        public Starshards(IEnumerable<ushort> views, char cardType, IEnumerable<ushort> cards) : base()
        {
            if (views != null)
                Set("views", views.ToList());
        ...
        {
            List<ushort> views = GetUshortArray("views");
            return Head + "," + (views == null ?
                    "0" : (views.Count + "," + string.Join(",", views))) + "," +
                    GetString("cardType") + "," + string.Join(",", GetUshortArray("cards"));
        }
        public static Starshards Parse(string message)
        {
            string[] g2qu = message.Split(',');
            int viewCount = int.Parse(g2qu[1]);
            ushort[] views = Util.TakeRange(g2qu, 2, 2 + viewCount)
                .Select(p => ushort.Parse(p)).ToArray();
            char cardType = g2qu[2 + viewCount][0];
            ushort[] cards = Util.TakeRange(g2qu, 3, g2qu.Length)
                .Select(p => ushort.Parse(p)).ToArray();
            return new Starshards(views, cardType, cards);
        }
        public override void Handle(XI xi)
        {
            List<ushort> views = GetUshortArray("views");
            ...
            List<ushort> cards = GetUshortArray("cards");
            ushort[] invs = (views ?? xi.Board.Garden.Keys).ToArray();
            xi.WI.Send("E0QU,0," + cardType + "," + string.Join(",", cards), invs);
            xi.WI.Send("E0QU,1," + cardType + "," + cards.Count, xi.ExceptStaff(invs));
            xi.WI.Live("E0QU,1," + cardType + "," + cards.Count);
        }
    }
