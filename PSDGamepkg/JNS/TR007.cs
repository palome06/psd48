...
namespace PSD.PSDGamepkg.JNS
{
    public partial class SkillCottage
    {
        ...
        public void JNT2202Action(Player player, int type, string fuse, string argst)
        {
            ...
            if (!eqed)
                XI.RaiseGMessage("G0HQ,0," + to + "," + who + ",1," + eq);
            else
            ...
            if (npc != null)
            {
                for (int i = 0; i < npc.Skills.Length; ++i)
                    if (npc.Skills[i] == "NJ01")
                    {
                        string nfuse = npc.Code + ";" + fuse;
                        if (XI.Nj01["NJ01"].Valid(player, nfuse))
                        {
                            inAble = true;
                            break;
                        }
        ...
        public string JNT2402Input(Player player, int type, string fuse, string prev)
        {
            ...
            {
                string head = "/I1" + (player.ROMCards.Count > 1 ? ("~" + player.ROMCards.Count) : "");
                return head + "(p" + string.Join("p", player.ROMCards) + ")";
            }
            else
                return "";
        }
        ...
        public void JNT2403Action(Player player, int type, string fuse, string argst)
        {
            ...
            string[] g0zhs = fuse.Split(',');
            List<Player> g0zhn = Util.TakeRange(g0zhs, 1, g0zhs.Length).Select(p =>
                 XI.Board.Garden[ushort.Parse(p)]).Where(p => p.IsAlive && p.Uid != tar).ToList();
            if (g0zhn.Count > 0)
                XI.InnerGMessage("G0ZH," + string.Join(",", g0zhn.Select(p => p.Uid)), -30);
        }
        public string JNT2403Input(Player player, int type, string fuse, string prev)
        {
            ...
            {
                string[] blocks = fuse.Split(',');
                var range = Util.TakeRange(blocks, 1, blocks.Length);
                List<Player> invs = range.Select(p => XI.Board.Garden[ushort.Parse(p)])
                    .Where(p => p.IsTared && p.HP == 0).ToList();
        ...
        public bool JNH0701Valid(Player player, int type, string fuse)
        {
            ...
            {
                bool stucken = (XI.Board.IsAttendWar(player) && player.ROMCards.Count > 0) ||
                    player.ROMCards.Any(p => XI.LibTuple.NL.Decode(
                    (ushort)(int.Parse(p.Substring("M".Length)) - 1000)).STR >= 5);
                return stucken;
            }
            else if (type == 2) // AF
            {
                bool waken = player.ROMCards.Any(p => XI.LibTuple.NL.Decode(
                    (ushort)(int.Parse(p.Substring("M".Length)) - 1000)).STR < 5);
            ...
            else if (type == 3 || type == 4) // IS/OS
            {
                ...
                {
                    return (XI.Board.IsAttendWar(player) && player.ROMCards.Count > 0) ||
                    player.ROMCards.Any(p => XI.LibTuple.NL.Decode(NMBLib.OriginalNPC(
        ...
        public void JNH0701Action(Player player, int type, string fuse, string argst)
        {
            ...
            {
                int upfive = player.ROMCards.Count(p => XI.LibTuple.NL.Decode(
                    (ushort)(int.Parse(p.Substring("M".Length)) - 1000)).STR >= 5);
                int dnfive = player.ROMCards.Count; int delta;
            ...
            {
                int dnfive = player.ROMCards.Count(p => XI.LibTuple.NL.Decode(
                    (ushort)(int.Parse(p.Substring("M".Length)) - 1000)).STR < 5);
                if (XI.Board.IsAttendWar(player))
                {
                    player.RAMInt += dnfive;
            ...
            else if (type == 3)
            {
                int upfive = player.ROMCards.Count(p => XI.LibTuple.NL.Decode(
                    (ushort)(int.Parse(p.Substring("M".Length)) - 1000)).STR >= 5);
                int dnfive = player.ROMCards.Count;
        ...
        public bool JNH0702Valid(Player player, int type, string fuse)
        {
            if (XI.Board.Garden.Values.Where(p => p.IsTared && p.Team == player.Team &&
                !p.Immobilized).Select(p => p.Uid).Except(player.RAMPeoples).Any())
            {
        ...
        public string JNH0702Input(Player player, int type, string fuse, string prev)
        {
            if (prev == "")
                return "#横置,/T1(p" + string.Join("p", XI.Board.Garden.Values
                    ...
                    .Select(p => p.Uid).Except(player.RAMPeoples)) + ")";
            else
                return "";
        }
        ...
        public bool JNH0901Valid(Player player, int type, string fuse)
        {
            ...
            if (cands.Count >= 2) {
                string[] crs = new string[] { "TP02", "TP03", "TPT1" };
                // G0CC,A,0,B,KN,x1,x2;TF
                string[] g0cc = fuse.Split(',');
                string kn = g0cc[4];
                if (g0cc[1] != player.Uid.ToString() && crs.Contains(kn))
                    return true;
            }
            return false;
        }
        public void JNH0901Action(Player player, int type, string fuse, string argst)
        {
            ...
            ushort[] txs = Util.TakeRange(g0cc, 5, g0cc.Length).Select(p => ushort.Parse(p)).ToList();
        ...
        public bool JNH0902Valid(Player player, int type, string fuse)
        {
            ...
            else if (type == 1)
            {
                ...
                return player.RAMString != "" || player.RAMUshort != 0;
            return false;
        }
            ...
            if (type == 0)
            {
                ...
                if (player.IsAlive)
                    XI.RaiseGMessage("G0CC," + player.Uid + ",0," +
                        player.Uid + "," + player.RAMString + "," + ut + ";0," + fuse);
                player.RAMUshort = 1;
            }
            else if (type == 1)
            {
        ...
        public bool JNH0903Valid(Player player, int type, string fuse, string argst)
        {
            if (type == 0) // G0CC
            {
                string[] g0cc = Util.Substring(fuse, 0, fuse.IndexOf(';')).Split(',');
        ...
        {
            string[] g0zh = fuse.Split(',');
            return g0zh.Contains(player.Uid.ToString()) && XI.Board.Garden.Values.Any(p =>
                p.Uid != player.Uid && p.IsTared);
        }
        public void JNH1002Action(Player player, int type, string fuse, string argst)
        {
            ...
            List<ushort> zhs = fuse.Substring("G0ZH,".Length).Split(',').Select(p => ushort.Parse(p)).ToList();
            zhs.Remove(player.Uid);
            if (zhs.Count > 0)
                XI.InnerGMessage("G0ZH," + string.Join(",", zhs), 50);
        }
        ...
        {
            if (type == 0)
                return XI.Board.IsAttendWar(player) && (XI.Board.RestMonPiles.Count + XI.Board.RestMonDises.Count > 0);
            else if (type == 1 || type == 2)
            {
                string[] blocks = fuse.Split(',');
                if (blocks[1] == player.Uid.ToString() && blocks[2] == "1" && blocks[3] != "0")
                {
                    int agl = player.ROMCards.Select(p => XI.LibTuple.ML.Decode(
                        ushort.Parse(p.Substring("M".Length)))).Sum(p => p.AGL) / 2;
                    return agl != player.ROMUshort;
                };
            } else
                return false;
        }
        public void JNH1301Action(Player player, int type, string fuse, string argst)
        {
            if (type == 0)
            {
                if (player.ROMCards.Count >= 3)
                {
                    XI.RaiseGMessage("G0OJ," + player.Uid + ",1," + player.ROMCards.Count +
                         "," + string.Join(",", player.ROMCards));
                    XI.RaiseGMessage("G0ON," + player.Uid + ",M," + player.ROMCards.Count +
                         "," + string.Join(",", player.ROMCards.Select(p => p.Substring("M".Length))));
                    XI.RaiseGMessage("G2TZ,0," + player.Uid + "," + string.Join(",", player.ROMCards));
                }
                ushort pop = XI.Board.RestMonPiles.Dequeue();
                Monster mon = XI.LibTuple.NL.Decode(pop);
                XI.RaiseGMessage("G0YM,3," + pop + ",0");
                int delta = mon.STR / 2;
                if (delta > 0)
                    XI.RaiseGMessage("G0IA," + player.Uid + ",1," + delta);
                XI.RaiseGMessage("G2TZ," + player.Uid + ",0,M" + mon);
                XI.RaiseGMessage("G0IJ," + player.Uid + ",1,1,M" + mon);
            }
            else if (type == 1 || type == 2)
            {
                int agl = player.ROMCards.Select(p => XI.LibTuple.ML.Decode(
                ...
                player.ROMUshort = agl;
            }
        }
        public bool JNH1302Valid(Player player, int type, string fuse)
        {
            return fuse == "G0AF,0,0" && XI.Board.Rounder.Team == player.OppTeam;
        }
        ...
        public bool JNH1501Valid(Player player, int type, string fuse)
        {
            if (type == 0 || type == 1)
            {
                if (IsMathISOS("JNH1501", player, fuse))
                {
                    foreach (Player py in XI.Board.Garden.Values)
                    {
                        if (py.Uid != player.Uid && py.IsAlive && p.Team == player.Team)
                        {
        ...
        public void JNT1502Action(Player player, int type, string fuse, string argst)
        {
            if (type == 0) {
                ...
                Cure(player.Uid, player.Uid, 1);
            } else if (type == 1) {
                int delta = player.ROMCards.Sum(p => XI.LibTuple.ML.Decode(
                        p => ushort.Parse(p.Substring("M".Length))).STR + 1);
                if (player.Team == 1)
        ...
        public string JNT1502Input(Player player, int type, string fuse, string prev)
        {
            if (type == 0) {
                if (prev == "")
        ...
        public void JNH1101Action(Player player, int type, string fuse, string argst)
        {
            ...
            {
                List<ushort> getc = XI.Board.Garden.Values.Where(p =>
                    p.Team == player.OppTeam && p.IsTared && p.Tux.Count > 0).Select(p => p.Uid).ToList();
                if (getc.Count > 0)
                {
                    string input = XI.AsyncInput(player.Uid, "#获得1张手牌,T1(p" +
                        string.Join("p", getc) + ")", "JNH1101", "0");
                    if (input != VI.CinSentinel)
                    {
                        ushort tar = ushort.Parse(input);
                        string c0 = Util.RepeatString("p0", XI.Board.Garden[tar].Tux.Count);
                        XI.AsyncInput(player.Uid, "#获得的,C1(" + c0 + ")", "JNH1101" , "0");
                        XI.RaiseGMessage("G0HQ,0," + player.Uid + "," + tar + ",2,1");
                    }
                }
            }
        }
        ...
        {
            if (type == 0)
                return player.Tux.Count > 0 && player.Tux.Count <= player.DEX;
            else if (type == 1 || type == 2 || type == 3)
                return player.ROMFolder.Count > 0;
            else
                return false;
        }
        public void JNH1102Action(Player player, int type, string fuse, string argst)
        {
            ...
            {
                XI.RaiseGMessage("G0IA," + player.Uid + ",1," + player.ROMFolder.Count);
                XI.RaiseGMessage("G0OX," + player.Uid + ",1," + player.ROMFolder.Count);
            }
            else if (type == 2 || type == 3)
            {
                List<ushort> ijs = player.ROMFolder.ToList();
        ...
        public bool JNH1703Valid(Player player, int type, string fuse)
        {
            if (player.Tux.Count > 0 && player.HP > 0)
            {
                string[] blocks = fuse.Split(',');
                return Util.TakeRange(blocks, 1, blocks.Length).Select(p =>
                     XI.Board.Garden[ushort.Parse(p)]).Any(p => p.Uid != player.Uid && p.IsTared && p.HP == 0);
            } else
                return false;
        }
        ...
        public string JNH1703Input(Player player, int type, string fuse, string prev)
        {
            if (prev == "")
            {
                ...
                var range = Util.TakeRange(blocks, 1, blocks.Length);
                List<Player> invs = range.Select(p => XI.Board.Garden[ushort.Parse(p)])
                    .Where(p => p.Uid != player.Uid && p.IsTared && p.HP == 0).ToList();
                return "/T1(p" + string.Join("p", invs.Select(p => p.Uid)) + ")";
            } else
                return "";
        }
        ...
        public void JNH1803Action(Player player, int type, string fuse, string argst)
        {
            ...
            else if (type == 1)
            {
                ...
                XI.RaiseGMessage("G0OE," + player.Uid);
                player.ROMUshort = 2;
            }
        }
        ...
        public bool JNH1902Valid(Player player, int type, string fuse)
        {
            if (type == 0)
            {
                return player.RAMUshort == 0 && player.ROMToken >= 2 &&
        ...
        public bool JNH1903Valid(Player player, int type, string fuse)
        {
            ...
            else if (type == 1 || type == 2)
            {
                if (player.ROMPlayerTar.Count > 0)
                {
                    ...
                    ushort tar = player.ROMPlayerTar[0];
        ...
        public void JNH1903Action(Player player, int type, string fuse, string argst)
        {
            if (type == 0)
            {
                ...
                if (py.STR > player.STR)
                    XI.RaiseGMessage("G0IA," + player.Uid + ",2," + (py.STR - player.STR));
                else if (py.STR < player.STR)
                    XI.RaiseGMessage("G0OA," + player.Uid + ",2," + (player.STR - py.STR));
            ...
            else if (type == 1)
            {
                ...
                if (py.STR > player.STR)
                    ...
                    XI.RaiseGMessage("G0IA," + py.Uid + ",2," + (player.STR - py.STR));
            }
            else if (type == 2)
            {
                ushort tar = player.ROMPlayerTar[0];
                Player py = XI.Board.Garden[tar];
                if (py.STR > player.STR)
                    XI.RaiseGMessage("G0OA," + player.Uid + ",2," + (py.STR - player.STR));
                else if (py.STR < player.STR)
                    XI.RaiseGMessage("G0OA," + py.Uid + ",2," + (player.STR - py.STR));
            }
            else if (type == 3)
            {
                if (player.ROMPlayerTar.Count > 0)
