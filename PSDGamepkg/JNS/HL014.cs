...
namespace PSD.PSDGamepkg.JNS
{
    public partial class SkillCottage
    {
        ...
        public void JNH0101Action(Player player, int type, string fuse, string argst)
        {
            ...
            if (cnt > 0)
                XI.RaiseGMessage(new Artiad.IncrTokenCount() { Who = player.Uid, Delta = cnt }.ToMessage());
        }
        ...
        public void JNH0102Action(Player player, int type, string fuse, string argst)
        {
            ...
            XI.RaiseGMessage(new Artiad.DecrTokenCount() { Who = player.Uid, Delta = cnt }.ToMessage());
            if (XI.Board.IsAttendWarSucc(player))
                XI.RaiseGMessage("G0IP," + player.Team + "," + (cnt + 3));
            else
        ...
        public void JNH0201Action(Player player, int type, string fuse, string argst)
        {
            ...
            else if (type == 2 && player.SingleTokenTar != 0)
            {
                XI.Board.Garden[player.SingleTokenTar].SetAllTuxDisabled("JNH0206", false);
                XI.RaiseGMessage(new Artiad.DecrTokenTar()
                {
                    Who = player.Uid,
                    SingleTar = player.SingleTokenTar
                }.ToMessage());
            }
        }
        ...
        public void JNH0206Action(Player player, int type, string fuse, string argst)
        {
            ...
            XI.RaiseGMessage(new Artiad.IncrTokenTar() { Who = player.Uid, SingleTar = ut }.ToMessage());
        ...
        public void JNH0402Action(Player player, int type, string fuse, string argst)
        {
            if (type == 0)
            {
                XI.RaiseGMessage(new Artiad.IncrTokenAwake() { Who = player.Uid }.ToMessage());
            ...
            {
                XI.RaiseGMessage(new Artiad.DecrTokenAwake() { Who = player.Uid }.ToMessage());
        ...
        public void JNH0503Action(Player player, int type, string fuse, string argst)
        {
            ...
            if (type == 0)
            {
                ...
                XI.RaiseGMessage(new Artiad.IncrTokenTar() { Who = player.Uid, SingleTar = tar }.ToMessage());
                XI.Board.Garden[tar].SetSilence("JNH0503");
            }
            else if (type == 1)
            {
                Player py = XI.Board.Garden[player.SingleTokenTar];
                py.ResetSilence("JNH0503");
                XI.RaiseGMessage(new Artiad.DecrTokenTar() { Who = player.Uid, SingleTar = py.Uid }.ToMessage());
            }
            else if (type == 2)
            {
                List<Artiad.Harm> harms = Artiad.Harm.Parse(fuse);
        ...
        public void JNH0701Action(Player player, int type, string fuse, string argst)
        {
            if (type == 0)
            {
                ...
                int rest = System.Math.Max(npc.STR, player.TokenExcl.Count);
        ...
        public void JNH0801Action(Player player, int type, string fuse, string argst)
        {
            ...
            int n = XI.Board.Garden.Values.Count(p => p.IsAlive && p.Immobilized);
            if (n > 0)
                XI.RaiseGMessage("G0DH," + string.Join(",", invs.Select(p => p.Uid + ",0," + n)));
            ...
            foreach (ushort who in XI.Board.OrderedPlayer())
            {
                Player py = XI.Board.Garden[who];
                if (py.IsAlive && invs.Contains(py))
                {
                    List<ushort> canUse = py.Tux.Where(p => canUseRule(py, p)).ToList();
                    string uses = XI.AsyncInput(who, canUse.Count > 0 ?
                        ("#使用,/Q1(p" + string.Join("p", canUse) + ")") : "/", "JNH0801", "0");
        ...
        public bool JNH0903Valid(Player player, int type, string fuse)
        {
            ...
            else if (type == 3 || type == 4) // R*TM/R*ED
        ...
        public void JNH0903Action(Player player, int type, string fuse, string argst)
        {
            if (type == 0)
            {
                ...
                if (g0cc[5] == "0")
                    --n;
                XI.RaiseGMessage(new Artiad.IncrTokenCount() { Who = player.Uid, Delta = n }.ToMessage());
            }
            else if (type == 1)
            {
                string[] g1di = fuse.Split(',');
                ...
                XI.RaiseGMessage(new Artiad.IncrTokenCount() { Who = player.Uid, Delta = 1 }.ToMessage());
            else if (type == 3)
            {
                XI.RaiseGMessage(new Artiad.DecrTokenCount() { Who = player.Uid, Delta = player.TokenCount }.ToMessage());
        ...
        public bool JNH1102Valid(Player player, int type, string fuse)
        {
            ...
            else if (type == 1 && player.TokenFold.Count > 0 && player.RAM.GetBool("Scared"))
            {
                int idx = fuse.IndexOf(';');
                string[] g0cc = fuse.Substring(0, idx).Split(',');
                ushort trigger = ushort.Parse(g0cc[3]);
                ...
                return player.TokenFold.Count > 0 || player.RAM.GetBool("Scared");
            else
                return false;
        }
        public void JNH1102Action(Player player, int type, string fuse, string argst)
        {
            if (type == 0)
            {
                ushort[] ijs = player.Tux.ToArray();
                XI.RaiseGMessage(new Artiad.IncrTokenFold() { Who = player.Uid, Delta = ijs }.ToMessage());
                XI.RaiseGMessage("G0OT," + player.Uid + "," + Algo.ListToString(ijs));
            ...
            else if (type == 1)
            {
                ...
                ushort[] txs = Algo.TakeRange(g0cc, 5, g0cc.Length).Select(p => ushort.Parse(p)).ToArray();
                ushort trigger = ushort.Parse(g0cc[3]);
                TargetPlayer(player.Uid, trigger);
                ...
                if (txs.Length > 0)
                {
                    ...
                    if (trigger == 0 || XI.LibTuple.TL.EncodeTuxCode(cardname).Type != Tux.TuxType.ZP)
                    {
                        XI.RaiseGMessage(new Artiad.Abandon()
                        {
                            Zone = Artiad.CustomsHelper.ZoneType.IMPLICIT,
                            Genre = Card.Genre.Tux,
                            SingleUnit = new Artiad.CustomsUnit() { Cards = txs.ToArray() }
                        }.ToMessage());
                    }
                    else
                    {
                        player.ROM.GetOrSetDiva("Forbidden").Set("source", trigger);
                        player.ROM.GetOrSetDiva("Forbidden").GetOrSetUshortArray("tux").AddRange(txs);
                    }
                }
                int kdx = fuse.IndexOf(',', hdx);
                if (kdx >= 0)
            ...
            else if (type == 2 || type == 3)
            {
                ...
                {
                    ushort[] ijs = player.TokenFold.ToArray();
                    XI.RaiseGMessage(new Artiad.DecrTokenFold() { Who = player.Uid, Delta = ijs }.ToMessage());
                    XI.RaiseGMessage("G0IT," + player.Uid + "," + Algo.ListToString(ijs));
                }
                player.RAM.Set("Scared", null);
                // Artiad.Procedure.SetPlayerAllEqEnable(XI, new Player[] { player }, 0x1, "JNH1102");
                ushort source = player.ROM.GetOrSetDiva("Forbidden").GetUshort("source");
                if (source != 0)
                {
                    List<ushort> txs = player.ROM.GetOrSetDiva("Forbidden").GetOrSetUshortArray("tux");
                    if (XI.Board.Garden[source].IsAlive)
                        XI.RaiseGMessage("G0HQ,2," + source + ",0,0," + string.Join(",", txs));
                    else
                    {
                        XI.RaiseGMessage(new Artiad.Abandon()
                        {
        ...
        {
            if (Artiad.LittleHelper.IsExcl(fuse))
            {
                if (type == 0)
                    return Artiad.IncrTokenExcl.Parse(fuse).Who == player.Uid;
                else if (type == 1)
                    return Artiad.DecrTokenExcl.Parse(fuse).Who == player.Uid;
                else return false;
            }
            else return false;
        }
        public void JNH1302Action(Player player, int type, string fuse, string argst)
        {
            int agl = (player.TokenExcl.Select(p => XI.LibTuple.ML.Decode(
                    ushort.Parse(p.Substring("M".Length)))).Sum(p => p.AGL) + 1) / 2;
            ...
            if (type == 0)
            {
                XI.RaiseGMessage("G0IA," + player.Uid + ",0," +
                    Artiad.IncrTokenExcl.Parse(fuse).Delta.Length);
            }
            else if (type == 1)
            {
                XI.RaiseGMessage("G0OA," + player.Uid + ",0," +
                    Artiad.DecrTokenExcl.Parse(fuse).Delta.Length);
            }
        }
        public bool JNH1303Valid(Player player, int type, string fuse)
        {
            if (type == 0)
            {
            ...
            else if (type == 1 && XI.Board.RoundIN == "R" + XI.Board.Rounder.Uid + "NP")
            {
        ...
        public void JNH1303Action(Player player, int type, string fuse, string argst)
        {
        ...
        public bool JNH1602Valid(Player player, int type, string fuse)
        {
            ...
            {
                return XI.Board.Rounder.Team == player.OppTeam && XI.Board.Garden.Values.Any(p =>
                    p.IsTared && p.HP == 0 && (p.Uid == player.Uid ||
                    XI.LibTuple.HL.InstanceHero(p.SelectHero).Bio.Contains("H")));
            }
        ...
        public void JNH1602Action(Player player, int type, string fuse, string argst)
        {
            if (type == 0)
            {
                ...
                if (player.Guardian != 0)
                {
                    XI.RaiseGMessage("G0MA," + player.Uid + ",0");
                    int ki = 0;
                    if (guardian >= 10 && guardian <= 14)
                        ki = guardian - 10 + 3;
                    if (guardian >= 15 && guardian <= 17)
                        ki = guardian - 15 + 10;
                    else if (guardian >= 20 && guardian <= 21)
                        ki = guardian - 20 + 8;
                    if (ki != 0)
                    {
        ...
        public bool JNH1612BKValid(Player player, int type, string fuse, ushort owner)
        {
            if (player.Uid == XI.Board.Rounder.Uid)
            {
                if (type == 0 && player.Tux.Count > 0)
                {
                    Tux copiee = XI.LibTuple.TL.EncodeTuxCode("JPR1");
                    if (!XI.LibTuple.TL.IsTuxInGroup(copiee, XI.PCS.Level))
                        copiee = XI.LibTuple.TL.EncodeTuxCode("JPT1");
                    return copiee.Bribe(player, 1, fuse) && copiee.Valid(player, 1, fuse);
                }
                else if (type == 1)
                    return true;
            }
        ...
        public void JNH1612Action(Player player, int type, string fuse, string argst)
        {
            if (type == 0)
            {
                ...
                XI.RaiseGMessage("G0CC," + player.Uid + ",0," +
                    player.Uid + "," + copiee.Code + "," + ut + ";1," + fuse);
            }
            else if (type == 1)
                Cure(player, XI.Board.Garden.Values.Where(p => XI.Board.IsAttendWar(p)), 1);
        }
        public string JNH1612Input(Player player, int type, string fuse, string prev)
        {
            if (type == 0 && prev.IndexOf(',') < 0)
            {
                List<ushort> tuxes = player.Tux.Where(p =>
                        ...
                        if (Base.Card.NMBLib.IsMonster(newTangle))
                        {
                            ...
                            if (bombs.Count > 0)
                                ans2nd = XI.AsyncInput(player.Uid, "#爆发,/M1(p" +
                                    string.Join("p", bombs) + ")", "JNH1802", "1");
                            else
                                ans2nd = XI.AsyncInput(player.Uid, "/", "JNH1802", "1");
        ...
        {
            if (type == 0)
                XI.RaiseGMessage(new Artiad.IncrTokenCount() { Who = player.Uid, Delta = 5 }.ToMessage());
            else if (type == 1 || type == 2)
                XI.RaiseGMessage("G0ZW," + player.Uid);
        }
        public bool JNH1902Valid(Player player, int type, string fuse)
        {
            ...
            {
                System.Func<Player, bool> yes = p => p.IsReal && p.Team == player.OppTeam && p.Gender == 'F';
                return player.TokenAwake && (yes(XI.Board.Supporter) || yes(XI.Board.Hinder));
            }
            else if (type == 2 && Artiad.LittleHelper.IsAwake(fuse)) // OJ
                return Artiad.DecrTokenAwake.Parse(fuse).Who == player.Uid;
            else if (type == 3) // IY
            {
                ...
                for (int i = 1; i < g0iy.Length;)
                {
                    ...
                    int heroNum = ushort.Parse(g0iy[i + 2]);
                    if (XI.Board.Garden[who].Team == player.OppTeam &&
                            XI.LibTuple.HL.InstanceHero(heroNum).Gender == 'F')
                        return true;
                    if (utype == 2)
                        i += 4;
                    else
        ...
        public void JNH1902Action(Player player, int type, string fuse, string argst)
        {
            ...
            {
                XI.RaiseGMessage(new Artiad.IncrTokenAwake() { Who = player.Uid }.ToMessage());
                XI.Board.Garden.Values.Where(p => p.IsAlive && p.Team == player.OppTeam && p.Gender == 'F')
                    .ToList().ForEach(py =>
                    {
                        py.SetJPDisabled("JNH1902", true);
                        py.SetTPDisabled("JNH1902", true);
                        py.SetZPDisabled("JNH1902", true);
                    });
                XI.RaiseGMessage(new Artiad.Goto() { Terminal = "R" + player.Uid + "Z0" }.ToMessage());
            }
            else if (type == 1)
            {
                foreach (ushort ut in XI.Board.OrderedPlayer())
                {
                    Player py = XI.Board.Garden[ut];
                    if (player.TokenAwake && py.IsAlive && py.Team == player.OppTeam && py.Gender == 'F')
                    {
                        if (py.Uid != XI.Board.Supporter.Uid && py.Uid != XI.Board.Hinder.Uid)
                            continue;
                        TargetPlayer(player.Uid, ut);
                        if (py.Uid == XI.Board.Supporter.Uid)
                            XI.RaiseGMessage(new Artiad.CoachingSign()
                            {
                                SingleUnit = new Artiad.CoachingSignUnit()
                                {
                        ...
                        else if (py.Uid == XI.Board.Hinder.Uid)
                            XI.RaiseGMessage(new Artiad.CoachingSign()
                            {
                                SingleUnit = new Artiad.CoachingSignUnit()
                                {
                        ...
                        if (py.Tux.Count > 0)
                        {
                            XI.RaiseGMessage("G0DH," + ut + ",1,1");
                            XI.RaiseGMessage("G0TT," + ut);
                            int value = XI.Board.DiceValue;
                            if (value < 5)
                            {
                                TargetPlayer(ut, player.Uid);
                                XI.RaiseGMessage(new Artiad.DecrTokenAwake() { Who = player.Uid }.ToMessage());
                                XI.RaiseGMessage(new Artiad.DecrTokenCount() { Who = player.Uid, Delta = 1 }.ToMessage());
                            }
                        }
                    }
                }
            ...
            else if (type == 2)
            {
                XI.Board.Garden.Values.Where(p => p.IsAlive && p.Team == player.OppTeam && p.Gender == 'F')
                    .ToList().ForEach(py =>
                    {
                        py.SetJPDisabled("JNH1902", false);
                        py.SetTPDisabled("JNH1902", false);
                        py.SetZPDisabled("JNH1902", false);
                    });
            }
            else if (type == 3)
            {
                ...
                for (int i = 1; i < g0iy.Length;)
                {
                    ...
                    if (XI.Board.Garden[who].Team == player.OppTeam &&
                            XI.LibTuple.HL.InstanceHero(heroNum).Gender == 'F')
                    {
                        XI.Board.Garden[who].SetJPDisabled("JNH1902", true);
                        XI.Board.Garden[who].SetTPDisabled("JNH1902", true);
                        XI.Board.Garden[who].SetZPDisabled("JNH1902", true);
                    }
                    if (utype == 2)
                        i += 4;
                    else
            ...
            else if (type == 4)
            {
                ...
                for (int i = 1; i < g0oy.Length; i += 2)
                {
                    ...
                    if (py.Gender == 'F')
                    {
                        XI.Board.Garden[who].SetJPDisabled("JNH1902", false);
                        XI.Board.Garden[who].SetTPDisabled("JNH1902", false);
                        XI.Board.Garden[who].SetZPDisabled("JNH1902", false);
                    }
                }
            }
        }
        ...
        public void JNH1903Action(Player player, int type, string fuse, string argst)
        {
            ...
            {
                if (player.SingleTokenTar != 0)
                    XI.RaiseGMessage(new Artiad.DecrTokenTar() { Who = player.Uid, SingleTar = player.SingleTokenTar }.ToMessage());
            }
        }
        ...
        public void JNH2001Action(Player player, int type, string fuse, string argst)
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                ...
                if (select != VI.CinSentinel)
                {
                    ...
                    XI.RaiseGMessage(new Artiad.IncrTokenFold() { Who = player.Uid, SingleDelta = ut }.ToMessage());
                }
            }
        }
        ...
        public void JNH2002Action(Player player, int type, string fuse, string argst)
        {
            ...
            if (type == 0)
            {
                XI.RaiseGMessage(new Artiad.DecrTokenFold()
                {
                    Who = player.Uid,
                    SingleDelta = ushort.Parse(argst),
                    Public = true
                }.ToMessage());
                XI.RaiseGMessage(new Artiad.Abandon()
                {
        ...
        #endregion E3W07 (01) - Sitang
        #region EX301 (02) - JingTian
        ...
        #endregion EX301 (02) - JingTian
        #region EX408 (03) - Chanyou
        ...
        public void JNE0301Action(Player player, int type, string fuse, string prev)
        {
            if (type == 0)
            {
                ...
                {
                    Who = player.Uid,
                    Delta = uts.Select(p => "C" + p).ToArray(),
                    Public = true
                }.ToMessage());
            }
            else if (type == 1)
        ...
        #endregion EX408 (03) - Chanyou
        #region EX506 (04) - Kumu
        ...
        public void JNE0402Action(Player player, int type, string fuse, string argst)
        {
            if (type == 0)
            {
                ...
                XI.RaiseGMessage(new Artiad.IncrTokenTar() { Who = player.Uid, Tars = sets.ToArray() }.ToMessage());
            }
            else if (type == 1)
            {
                string[] g0ht = fuse.Split(',');
        ...
        #endregion EX506 (04) - Kumu
        #region EX515 (05) - Qingshi
        public bool JNE0501Valid(Player player, int type, string fuse)
        {
            if (type == 0) // G0CC
            {
                ...
                if (g0cc[5] == "0")
                    --n;
                return XI.Board.RoundIN == "R" + who + "GR" && who == XI.Board.Rounder.Uid &&
                    XI.Board.Rounder.Team == player.OppTeam && n > 0;
            }
            else if (type == 1)
            {
                string[] g1ue = fuse.Split(',');
                ushort who = ushort.Parse(g1ue[1]);
                return XI.Board.RoundIN == "R" + who + "GR" && who == XI.Board.Rounder.Uid &&
                    XI.Board.Rounder.Team == player.OppTeam;
            }
            else if (type == 2)
                return player.Tux.Count > 0 && player.TokenCount > 0;
            else if (type == 3)
                return player.TokenCount > 0;
            else return false;
        }
        public void JNE0501Action(Player player, int type, string fuse, string argst)
        {
            ...
            {
                ushort[] show = argst.Split(',').Select(p => ushort.Parse(p)).ToArray();
                List<ushort> others = player.Tux.Except(show).ToList();
                if (others.Count > 0)
                    XI.RaiseGMessage("G0QZ," + player.Uid + "," + string.Join(",", others));
                XI.RaiseGMessage("G2FU,0,0,0,C," + string.Join(",", show));

                Player r = XI.Board.Rounder;
                var tuxTypes = show.Select(p =>
                    XI.LibTuple.TL.DecodeTux(p).Type).Distinct().ToList();
                List<ushort> gives = r.ListOutAllCards().Where(p =>
                    tuxTypes.Contains(XI.LibTuple.TL.DecodeTux(p).Type)).ToList();
                string decision = XI.AsyncInput(r.Uid, gives.Count == 0 ? "/" :
                    "#弃置,/Q1(p" + string.Join("p", gives) + ")", "JNE0501", "0");
                if (!decision.StartsWith("/"))
                {
                    XI.RaiseGMessage("G0QZ," + r.Uid + "," + decision);
                    XI.RaiseGMessage(new Artiad.DecrTokenCount() { Who = player.Uid, Delta = player.TokenCount }.ToMessage());
                }
                else
                {
                    XI.RaiseGMessage("G0IX," + player.Uid + ",0," + player.TokenCount);
                    player.RFM.Set("Caught", true);
                }
            }
            else if (type == 3)
            {
                if (player.RFM.GetBool("Caught"))
                    XI.RaiseGMessage("G0OX," + player.Uid + ",0," + player.TokenCount);
                XI.RaiseGMessage(new Artiad.DecrTokenCount() { Who = player.Uid, Delta = player.TokenCount }.ToMessage());
            }
        }
        public string JNE0501Input(Player player, int type, string fuse, string prev)
        {
            if (type == 2 && prev == "")
            {
                return "#展示,/Q1" + (player.Tux.Count > 1 ? ("~" + player.Tux.Count) : "") +
                    "(p" + string.Join("p", player.Tux) + ")";
            }
            else return "";
        }
        public bool JNE0502Valid(Player player, int type, string fuse)
        {
            ...
            else if (type == 1 || type == 2)
            {
                string[] blocks = fuse.Split(',');
                ushort who = ushort.Parse(blocks[1]);
                return who == player.Uid && XI.Board.PoolEnabled;
            }
            else if (type == 3)
            {
                return IsMathISOS("JNE0502", player, fuse) &&
                    player.DEX > 0 && XI.Board.PoolEnabled;
            }
            else return false;
        }
        public void JNE0502Action(Player player, int type, string fuse, string argst)
        {
            int point = player.DEX;
            if (type >= 0 && type <= 2)
                XI.RaiseGMessage("G1WP," + player.Team + "," + player.Uid + ",JNE0502," + point);
            else if (type == 3)
                XI.RaiseGMessage("G1WP," + player.Team + "," + player.Uid + ",JNE0502,0");
        }
        public bool JNE0503Valid(Player player, int type, string fuse)
        {
            return XI.Board.Battler != null && XI.Board.Battler.AGL - player.DEX <= 2;
        }
        public void JNE0503Action(Player player, int type, string fuse, string argst)
        {
            string select = XI.AsyncInput(player.Uid, "#请选择『封手』执行项" +
                "##补1牌##出场无效,Y2", "JNE0503", "0");
            XI.SendOutUAMessage(player.Uid, "JNE0503," + select, "0");
            if (select == "2")
            {
        ...
        #endregion EX515 (05) - Qingshi
        #region EX119 (07) - Jisan'niang
        ...
        public void JNE0701Action(Player player, int type, string fuse, string argst)
        {
            ...
            XI.AsyncInput(player.Uid, "#弃置的,C1(" + c0 + ")", "JNE0701", "1");
        ...
        #endregion EX119 (07) - Jisanniang
        #region EX312 (08) - ZhaoWuyan
        ...
        public bool JNE0803Valid(Player player, int type, string fuse)
        {
            ...
            {
                return XI.Board.Garden.Values.Any(p => p.HP == 0 && p.Uid != player.Uid && p.IsTared) &&
        ...
        public void JNE0803Action(Player player, int type, string fuse, string argst)
        {
            if (type == 0)
            {
                ...
                {
                    Who = player.Uid,
                    SingleDelta = "M" + outSoul,
                    Public = true
                }.ToMessage());
                XI.RaiseGMessage(new Artiad.IncrTokenExcl()
                {
                    Who = player.Uid,
                    SingleDelta = "M" + inSoul,
                    Public = true
                }.ToMessage());
                int delta = 0;
        ...
        #endregion EX312 (08) - ZhaoWuyan
        #region EX116 (10) - Elder Shi
        public bool JNE1001BKValid(Player player, int type, string fuse, ushort owner)
        {
            return XI.Board.Garden[owner].Team == player.Team && player.Tux.Count > 0;
        }
        public void JNE1001Action(Player player, int type, string fuse, string argst)
        {
            int idx = argst.IndexOf(',');
            Player owner = XI.Board.Garden[ushort.Parse(argst.Substring(0, idx))];
            ushort ut = ushort.Parse(argst.Substring(idx + 1));
            Tux tux = XI.LibTuple.TL.DecodeTux(ut);
            XI.RaiseGMessage("G0CC," + player.Uid + ",0," +
                player.Uid + "," + tux.Code + "," + ut + ";0," + fuse);
            Harm(owner, owner, 2, FiveElement.AGNI);
        }
        public string JNE1001Input(Player player, int type, string fuse, string prev)
        {
            if (prev.IndexOf(',') < 0)
            {
                List<ushort> zps = player.Tux.Where(p =>
                    XI.LibTuple.TL.DecodeTux(p).Type == Tux.TuxType.ZP).ToList();
                return zps.Count > 0 ? "#使用战牌,/Q1(p" + string.Join("p", zps) + ")" : "/";
            }
            else return "";
        }
        public bool JNE1002Valid(Player player, int type, string fuse)
        {
            ...
            return mon != null && (mon.Element == FiveElement.AQUA || mon.Element == FiveElement.AGNI)
                && XI.Board.Garden.Values.Any(p => p.IsAlive && p.Team == player.Team && XI.Board.IsAttendWar(p));
        }
        public void JNE1002Action(Player player, int type, string fuse, string argst)
        {
            Harm(player, XI.Board.Garden.Values.Where(p =>
                p.IsAlive && p.Team == player.OppTeam), 1, FiveElement.AGNI);
        }
        public bool JNE1003Valid(Player player, int type, string fuse)
        {
            if (type == 0) return true;
            else if (type == 1 || type == 2) return player.IsAlive && player.TokenAwake;
            else return false;
        }
        public void JNE1003Action(Player player, int type, string fuse, string argst)
        {
            if (type == 0)
            {
                if (player.STR > 0)
                    XI.RaiseGMessage("G0IA," + player.Uid + ",0," + player.STR);
                if (player.DEX > 0)
                    XI.RaiseGMessage("G0IX," + player.Uid + ",0," + player.DEX);
                XI.RaiseGMessage(new Artiad.IncrTokenAwake() { Who = player.Uid }.ToMessage());
            }
            else if (type == 1 || type == 2)
            {
                Harm(player, XI.Board.Garden.Values.Where(p =>
                    p.IsAlive && p.Team == player.OppTeam), 2, FiveElement.AGNI);
                XI.RaiseGMessage("G0ZW," + player.Uid);
            }
        }
        #endregion Ex116 (10) - Elder Shi
        ...
        public void JNR0401Action(Player player, int type, string fuse, string args)
        {
            ...
            if (first.Length == 2)
                XI.RaiseGMessage("G0XZ," + player.Uid + ",1,1,2,1");
        ...
        public void JNR0501Action(Player player, int type, string fuse, string argst)
        {
            if (type == 0)
                ...
                {
                    Who = player.Uid,
                    Delta = new string[] { "I1", "I2", "I3", "I4", "I5" },
                    Public = true
                }.ToMessage());
            else if (type == 1)
            {
        ...
        #region RM405 (06) - Xuanxiao
        public bool JNR0601Valid(Player player, int type, string fuse)
        {
            if (type == 0)
            {
                ...
                return IsMathISOS("JNR0601", player, fuse);
            else if (type == 3)
                return player.TokenExcl.Count < FiveElementHelper.GetPropedElements().Length;
            else
                return false;
        }
        public void JNR0601Action(Player player, int type, string fuse, string argst)
        {
            if (type == 0)
            {
                List<Artiad.Harm> harms = Artiad.Harm.Parse(fuse);
        ...
        public string JNR0601Input(Player player, int type, string fuse, string prev)
        {
            if (type == 3 && prev == "")
            {
                int[] cands = FiveElementHelper.GetPropedElements().Select(p => p.Elem2Int()).Except(
                    player.TokenExcl.Select(p => int.Parse(p.Substring("V".Length)))).ToArray();
                return "/V1(p" + string.Join("p", cands) + ")";
            }
            else return "";
        }
        public bool JNR0602Valid(Player player, int type, string fuse)
        {
            if (type == 0)
                return IsMathISOS("JNR0602", player, fuse);
            else
                return JN50502Valid(player, type, fuse);
        }
        public void JNR0602Action(Player player, int type, string fuse, string argst)
        {
            if (type == 1)
            {
                ...
                JN50502Action(player, type, fuse, argst);
        }
        #endregion RM405 (06) - Xuanxiao
    }
}