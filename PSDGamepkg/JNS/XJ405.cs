...
namespace PSD.PSDGamepkg.JNS
{
    public partial class SkillCottage : JNSBase
    {
        ...
        public bool JN10401Valid(Player player, int type, string fuse)
        {
            if (type == 0)
            {
                return Artiad.EqImport.Parse(fuse).Imports.Any(p => p.Who == player.Uid &&
                    p.GetActualCardAs(XI).Type == Base.Card.Tux.TuxType.WQ);
            }
            else if (type == 1)
            {
        ...
        public void JN10401Action(Player player, int type, string fuse, string argst)
        {
            if (type == 0)
            {
                int n = Artiad.EqImport.Parse(fuse).Imports.Count(p => p.Who == player.Uid &&
                    p.GetActualCardAs(XI).Type == Base.Card.Tux.TuxType.WQ);
                XI.RaiseGMessage("G0IA," + player.Uid + ",0," + n);
            }
            else if (type == 1)
            {
                int n = Artiad.EqExport.Parse(fuse).Exports.Count(p => p.Who == player.Uid &&
                    p.GetActualCardAs(XI).Type == Base.Card.Tux.TuxType.WQ);
        ...
        public bool JN10601Valid(Player player, int type, string fuse) //I'C //O'C
        {
            if (type == 0)
            {
                return Artiad.EqImport.Parse(fuse).Imports.Any(p => p.Who == player.Uid &&
        ...
        public void JN10601Action(Player player, int type, string fuse, string argst)
        {
            if (type == 0)
            {
                int n = Artiad.EqImport.Parse(fuse).Imports.Count(p => p.Who == player.Uid &&
                    p.GetActualCardAs(XI).Type == Base.Card.Tux.TuxType.WQ);
                XI.RaiseGMessage("G0IX," + player.Uid + ",0," + n);
            }
            else if (type == 1)
            {
                int n = Artiad.EqExport.Parse(fuse).Exports.Count(p => p.Who == player.Uid &&
                    p.GetActualCardAs(XI).Type == Base.Card.Tux.TuxType.WQ);
                XI.RaiseGMessage("G0OX," + player.Uid + ",0," + n);
            }
        }
        public bool JN10602Valid(Player player, int type, string fuse) // Set as Lockin
        {
        ...
        public bool JN50101Valid(Player player, int type, string fuse)
        {
            if (type == 0 || type == 1 || type == 4)
            {
                ...
                // TODO: bug here, G0IA to others would trigger type = 0 and 1 together because the flag = false.
                // Should be solve by general judge of lock skills in XID/G
            }
        ...
        public bool JN60601Valid(Player player, int type, string fuse)
        {
            ...
            else if (type == 2 && Artiad.LittleHelper.IsExcl(fuse))
        ...
        public void JN60601Action(Player player, int type, string fuse, string argst)
        {
            if (type == 0)
            {
                ...
                {
                    Who = player.Uid,
                    Delta = heros.Select(p => "H" + p).ToArray(),
                    Public = true
                }.ToMessage());
        ...
        public void JN60602Action(Player player, int type, string fuse, string argst)
        {
            ...
            if (souls.Count > 0)
                ...
                {
                    Who = player.Uid,
                    Delta = souls.Select(p => "H" + p).ToArray(),
                    Public = true
                }.ToMessage());
            XI.RaiseGMessage("G0OY,0," + player.Uid);
        ...
        public void JN60801Action(Player player, int type, string fuse, string argst)
        {
            ...
            {
                Artiad.DecrTokenCount dtc = Artiad.DecrTokenCount.Parse(fuse);
                int zelta = player.TokenCount / 2 - (player.TokenCount - dtc.Delta) / 2;
        ...
        public bool JN60801Valid(Player player, int type, string fuse)
        {
            ...
            else if (type == 1 && Artiad.LittleHelper.IsCount(fuse))
            {
                Artiad.DecrTokenCount dtc = Artiad.DecrTokenCount.Parse(fuse);
                return dtc.Who == player.Uid && player.TokenCount / 2 - (player.TokenCount - dtc.Delta) / 2 > 0;
            }
            else return false;
        }
        ...
        public void JN60802Action(Player player, int type, string fuse, string argst)
        {
            ...
            foreach (Artiad.Harm harm in harms)
            {
                    ...
                    !HPEvoMask.DECR_INVAO.IsSet(harm.Mask))
                {
                    ...
                    if ((harm.N -= point) <= 0)
                        rvs.Add(harm);
                    XI.RaiseGMessage(new Artiad.DecrTokenCount() { Who = player.Uid, Delta = point }.ToMessage());
                }
            }
        ...
        public void JN60803Action(Player player, int type, string fuse, string argst)
        {
            ...
            if (count > 0)
            {
                XI.RaiseGMessage(new Artiad.DecrTokenCount()
                {
                    Who = player.Uid,
                    Delta = player.TokenCount
                }.ToMessage());
        ...
        public void JNS0102Action(Player player, int type, string fuse, string argst)
        {
            ...
            if (type == 0 || type == 1)
            {
                ...
                XI.RaiseGMessage("G2TZ," + player.Uid + ",0,H" + next);
        ...
        public void JNS0501Action(Player player, int type, string fuse, string argst)
        {
            if (type == 0)
            {
                ushort tr = ushort.Parse(argst);
                TargetPlayer(player.Uid, tr);
                XI.RaiseGMessage(new Artiad.IncrTokenTar() { Who = player.Uid, SingleTar = tr }.ToMessage());
                XI.Board.Garden[tr].SetZPDisabled("JNS0501", true);
            }
            else if (type == 1)
            {
                if (player.SingleTokenTar != 0)
                {
                    ushort tr = player.SingleTokenTar;
                    XI.RaiseGMessage(new Artiad.DecrTokenTar() { Who = player.Uid, SingleTar = tr }.ToMessage());
                    XI.Board.Garden[tr].SetZPDisabled("JNS0501", false);
                }
            }
        }
