                ...
                case "E0ZB":
                    {
                        ...
                        if (slot == 6)
                        {
                            ...
                            Z0D[me].Fakeq[card] = cardAs;
                            tuxText = zd.Tux(card); zoneText = "配饰区";
                        }
                        else if (slot == 5)
                        {
                            ushort[] cards = Algo.TakeRange(args, 4, args.Length)
                                .Select(p => ushort.Parse(p)).ToArray();
                            Z0D[me].ExCards.AddRange(cards);
                            tuxText = zd.Tux(cards);
                            zoneText = zd.HeroExCardAlias(Z0D[me].SelectHero, Z0D[me].Coss);
                        }
                        else
                        {
                            ushort card = ushort.Parse(args[4]);
                            tuxText = zd.Tux(card);
                            if (slot == 1) { Z0D[me].Weapon = card; zoneText = "武器区"; }
                            else if (slot == 2) { Z0D[me].Armor = card; zoneText = "防具区"; }
                            else if (slot == 3) { Z0D[me].Trove = card; zoneText = "秘宝区"; }
                            else if (slot == 4)
                            {
                                Z0D[me].ExEquip = card;
                                zoneText = zd.HeroExCardAlias(Z0D[me].SelectHero, Z0D[me].Coss);;
                ...
                case "E0SW":
                    {
                        ...
                        ushort who = ushort.Parse(args[2]);
                        Card.Genre pile = args[3][0].Char2Genre();

                        List<ushort> invs = Algo.TakeRange(args, 4,
                            args.Length).Select(p => ushort.Parse(p)).ToList();
                        if (invs.Count > 0 && actionType != 4)
                        {
                            string actionText = actionType == 2 ? "声明" : "展示";
                            string text = "";
                            switch (pile)
                            {
                                case Card.Genre.Tux: text = zd.Tux(invs); break;
                                case Card.Genre.NMB: text = zd.Monster(invs); break;
                                case Card.Genre.Eve: text = zd.Eve(invs); break;
                                case Card.Genre.TuxSerial: text = zd.TuxDbSerial(invs); break;
                                case Card.Genre.Rune: text = "标记" + zd.Rune(invs); break;
                                case Card.Genre.Five: text = "属性" + zd.Prop(invs.Select(p => (int)p)); break;
                            }
                ...
                case "E0FI":
                    ...
                    else
                    {
                        ...
                        while (idx < args.Length)
                        {
                            ...
                            {
                                ushort s = ushort.Parse(args[idx + 1]);
                                if (s != 0)
                                {
                                    Z0F.Drums.Add(s);
