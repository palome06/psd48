...
using BCard = PSD.Base.Card.Card;
...
namespace PSD.ClientAo
{
    public partial class XIVisi
    {
            ...
            new Thread(() => ZI.SafeExecute(() =>
            {
                while (true)
                {
                    ...
                    if (!string.IsNullOrEmpty(readLine))
                    {
                        ...
                        lock (unhandledMsg)
                        {
                            if (WI is VW.Eywi && readLine.Contains("|||"))
                            {
                                foreach (string line in Algo.Splits(readLine, "|||"))
                                    unhandledMsg.Enqueue(line);
                            }
                            else
        ...
        private string FormattedInputWithCancelFlag(string line)
        {
            ...
            repaint:
                ...
                else if (arg[0] == 'X') // Arrangement
                {
                    ...
                    inputValid &= Algo.IsSubSet(roundInput.Split(','), ussnm);
                    prevComment = ""; cancel = "";
                }
            ...
            switch (args[0])
            {
                ...
                case "E0FU":
                    ...
                    else if (args[1].Equals("2")) // 2:Show/Declare
                    {
                        ...
                        if (cardType == "C")
                            VI.Cout(Uid, "{0}展示了{1}.", who, zd.Tux(invs));
                        if (cardType == "M")
                            VI.Cout(Uid, "{0}展示了{1}.", who, zd.Monster(invs));
                        if (cardType == "E")
                            VI.Cout(Uid, "{0}展示了{1}.", who, zd.Eve(invs));
                        if (cardType == "G")
                    ...
                    else if (args[1].Equals("3")) // 3:Close
                        VI.OWatch(Uid, "E0FU");
                    else if (args[1].Equals("4")) // 4:Used only in replay to replace E0XZ
                    {
                        if (WI is VW.Eywi)
                        {
                            string cardType = args[2];
                    ...
                    else if (args[1].Equals("5")) // 5:Show G, legacy, should combined with 2
                    {
                        ushort who = ushort.Parse(args[2]);
                ...
                case "E0ZC":
                    {
                        ...
                        if (!hind)
                        {
                            string[] patstr = new string[]
                            {
                                ...
                                "",
                                "",
                                "{0}发动了{1}[{2}]特效{3},作用于{4}.",
                                "{0}爆发了{1}[{2}]{3},作用于{4}."
                            };
                            VI.Cout(Uid, patstr[consumeType], zd.Player(me),
                ...
                case "E0ZI":
                    {
                        ...
                        foreach (var pair in imc)
                            VI.Cout(Uid, "{0}爆发了{1}.", zd.Player(pair.Key), zd.Tux(pair.Value));
                    }
                    break;
                case "E0ZL":
                    {
                ...
                case "E0HI":
                    {
                        ...
                        foreach (var pair in imc)
                            VI.Cout(Uid, "{0}爆发了{1}.", zd.Player(pair.Key), zd.Monster(pair.Value));
                    }
                    break;
                case "E0HD":
                    {
                ...
                case "E0SW":
                    {
                        ushort actionType = ushort.Parse(args[1]);
                        ushort who = ushort.Parse(args[2]);
                        string piles = args[3];
                        BCard.Genre pile = piles[0].Char2Genre();

                        List<ushort> invs = Algo.TakeRange(args, 4,
                            args.Length).Select(p => ushort.Parse(p)).ToList();
                        if (invs.Count > 0)
                        {
                            if (actionType != 4)
                            {
                                string actionText = actionType == 2 ? "声明" : "展示";
                                string text = "";
                                switch (pile)
                                {
                                    case BCard.Genre.Tux: text = zd.Tux(invs); break;
                                    case BCard.Genre.NMB: text = zd.Monster(invs); break;
                                    case BCard.Genre.Eve: text = zd.Eve(invs); break;
                            ...
                            if (actionType == 1 || actionType == 4)
                                A0O.FlyingGet(invs.Select(p => piles + p), who, who, false);
                            else if (actionType == 2)
                                A0O.FlyingGet(invs.Select(p => piles + p), who, who, true);
                        }
                    }
                    break;
                case "E0AS":
                ...
                case "E0YZ":
                    {
                        BCard.Genre genre = BCard.Char2Genre(args[1][0]);
                        bool encrypted = args[2] == "1";
                        if (!encrypted)
                        {
                            ushort[] cards = Algo.TakeRange(args, 3, args.Length).Select(p => ushort.Parse(p)).ToArray();
                            if (genre == BCard.Genre.Tux)
                                A0O.FlyingGet(cards.Select(p => "C" + p).ToList(), 0, 0);
                            else if (genre == BCard.Genre.NMB || genre == BCard.Genre.NPC)
                                A0O.FlyingGet(cards.Select(p => "M" + p).ToList(), 0, 0);
                            else if (genre == BCard.Genre.Eve)
                                A0O.FlyingGet(cards.Select(p => "E" + p).ToList(), 0, 0);
                            else if (genre == BCard.Genre.Hero)
                                A0O.FlyingGet(cards.Select(p => "H" + p).ToList(), 0, 0);
                        }
                        else
                        {
                            int n = int.Parse(args[3]);
                            if (genre == BCard.Genre.Tux)
                                A0O.FlyingGet(Algo.RepeatToArray("C0", n), 0, 0);
                            else if (genre == BCard.Genre.NMB || genre == BCard.Genre.NPC)
                                A0O.FlyingGet(Algo.RepeatToArray("M0", n), 0, 0);
                            else if (genre == BCard.Genre.Eve)
                                A0O.FlyingGet(Algo.RepeatToArray("E0", n), 0, 0);
                            else if (genre == BCard.Genre.Hero)
                                A0O.FlyingGet(Algo.RepeatToArray("H0", n), 0, 0);
                        }
                    }
                    break;
                case "E0YB":
        ...
        public bool HandleV0Message(int uvsn, string invs, string mai)
        {
            ...
            {
                StartCinEtc();
                invs.Split(',').Select(p => ushort.Parse(p))
